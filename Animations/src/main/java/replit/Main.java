...
public class Main extends Application
{
    ...
    private int score;
    private Ball ball = new Ball(80, 2, 5);
    ...
    private void run(GraphicsContext gc)
    {
        ...
        if(ball.getY() < 0 || ball.getY() + ball.getR() > screenHeight)
        {
            ball.hitTopBotWall();
        }

        if(ball.getX() < 0 || ball.getX() + ball.getR() > screenWidth)
        {
            ball.hitSideWall();
            score++;
        }
        ...
        Image img = null;
            ...
            img = new Image(Main.class.getResource("image.png").toString());
        
        
        if(img != null)
        {
            //gc.drawImage(img, 250, 250);
            gc.drawImage(img, ball.getX(), ball.getY(), ball.getR(), ball.getR());
